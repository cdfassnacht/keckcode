...
Defines the NIRC2Set class, which is just a specialized version of the
CCDSet class, with a few add-ons that are specific to the NIRC2 imager
...
class NIRC2Set(CCDSet):
    ...
    def __init__(self, inlist, wcstype='koa', is_sci=True, texpkey='truitime',
        ...
        if isinstance(inlist, (list, tuple, dict)):
            ...
            raise TypeError('\nNIRC2Set: inlist must be either a list, a'
                            ' tuple, or a dict')
        ...
        if pyversion == 2:
            super(NIRC2Set, self).__init__(filelist, texpkey=texpkey,
